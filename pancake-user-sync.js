(function(){
  ...
  const VERSION='2.1.0-github-admin-product-detail-fallback';
  ...
  async function loadApi(){
    if(api)return api;
    api=await import('./pancake-browser-common.js?v=3');
    return api;
  }
    ...
    try{
      ...
      if(!rows.length){
        ...
        const sample=Array.isArray(m.detailFailureSamples)&&m.detailFailureSamples.length
          ...
          : '';
        throw new Error(`Pancake trả ${data.fetchedOrders||data.totalEntries||0} đơn nhưng chưa lấy được dòng sản phẩm.${detailInfo}${sample}`);
      }
